...
function ProductCard({ product, storePrice }) {
  const { user, addToCart, updateCartItem, selectedStore, cart } = useApp();
        ...
        <div className="product-card-pricing">
          ...
          {cartItem ? (
            <div className="product-card-qty" data-testid={`qty-control-${product.id}`} onClick={(e) => e.stopPropagation()}>
              <button className="qty-btn" onClick={(e) => { e.stopPropagation(); updateCartItem(cartItem.id, cartItem.quantity - 1); }} data-testid={`qty-minus-${product.id}`}>
                <Minus size={14} />
              </button>
              <span className="qty-value">{cartItem.quantity}</span>
              <button className="qty-btn qty-btn-plus" onClick={(e) => { e.stopPropagation(); updateCartItem(cartItem.id, cartItem.quantity + 1); }} data-testid={`qty-plus-${product.id}`}>
                <Plus size={14} />
              </button>
            </div>
          ) : (
